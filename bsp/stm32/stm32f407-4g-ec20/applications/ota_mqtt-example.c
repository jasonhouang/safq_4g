...
static rt_bool_t is_flash_writen_started = RT_FALSE;
...
static enum rym_code ota_on_write_flash_begin(rt_size_t total_len)
{
    update_file_total_size = total_len;
    rt_kprintf("ota file_size:%d\n", update_file_total_size);
...
#define OTA_BUF_LEN        (128)//(5000)
    ...
    do {
        ...
        if (IOT_OTA_IsFetching(h_ota)) {
            ...
            do {
#if 1
                if (!is_flash_writen_started)
                {
                    IOT_OTA_Ioctl(h_ota, IOT_OTAG_FILE_SIZE, &size_file, 4);
                    LOG_I("size_file = %d\n", size_file);
                    if (0)//RYM_CODE_NONE != ota_on_write_flash_begin(size_file))
                    {
                        LOG_E("ota_on_write_flash_begin failed\n");
                        goto do_exit;
                    }
                    is_flash_writen_started = RT_TRUE;
                }
#endif
#if 1
                ...
                if (len > 0) {
                    ...
                    if (fal_partition_write(dl_part, update_file_cur_size, buf_ota, len) < 0)
                    {
...
static int ota_example_main(int argc, char *argv[])
{
...
MSH_CMD_EXPORT_ALIAS(ota_example_main, ali_ota_sample, ali ota sample);
#endif
